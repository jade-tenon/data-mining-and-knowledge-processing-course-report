...
def evaluate(model, eval_loader, device):
    ...
    return total_loss / len(eval_loader), correct_predictions / total_predictions
...
def train(train_texts, train_labels, val_texts=None, val_labels=None):
    ...
    # 强制设置pad_token并同步扩展模型词表
    if tokenizer.pad_token is None or tokenizer.pad_token_id is None:
        tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        pad_token_added = True
        print(f"Added pad_token: {tokenizer.pad_token}, pad_token_id: {tokenizer.pad_token_id}")
...
if __name__ == "__main__":
    ...
    # 分别加载训练集、验证集和测试集
    print("加载训练集...")
    train_texts, train_labels = data_loader.load_csv("dataset/small_train.csv")
    print("加载验证集...")
