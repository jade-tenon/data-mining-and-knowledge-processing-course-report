...
def train(train_texts, train_labels, val_texts=None, val_labels=None):
    ...
    if device.type == 'cpu':
        print('警告：未检测到可用GPU，当前使用CPU训练，速度较慢。')
    else:
        print('检测到GPU，使用GPU训练。')
    ...
    for epoch in range(config.num_epochs):
        model.train()
        total_loss = 0
        batch_iter = tqdm(train_loader, desc=f'Epoch {epoch + 1}/{config.num_epochs}', ncols=100)
...
if __name__ == "__main__":
    ...
    # 分别加载训练集、验证集和测试集
    train_texts, train_labels = data_loader.load_csv("dataset/train_part3.csv")
